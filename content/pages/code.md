...
<p class="code-note">
  Selected software repositories connected to publications and research projects.
</p>
...
<section class="code-section">
  ...
  <div class="repo-list">
    ...
    {% for publication in publications_sorted %}
      {% assign code_url = publication.code_url | default: "" %}
      {% if code_url != "" %}
        {% assign repo_path = code_url | remove: "https://github.com/" | remove: "http://github.com/" %}
        <article class="repo-entry">
          <div>
            <h3><a href="{{ code_url }}">{{ repo_path }}</a></h3>
            <p>{{ publication.title }}</p>
            <span>{{ publication.venue }}</span>
          </div>
          <a class="publication-link" href="{{ code_url }}" aria-label="Open {{ repo_path }} on GitHub" title="Repo">
